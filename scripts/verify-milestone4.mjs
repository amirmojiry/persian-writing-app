...
for (const [path, needles] of required) {
  ...
  for (const needle of needles) {
    if (!content.includes(needle)) throw new Error(`${path} is missing required Milestone 4 marker: ${needle}`);
  }
}
...
for (const directory of ['packages/core/src', 'apps/client/src/components']) {
  for (const path of await filesBelow(new URL(`../${directory}/`, import.meta.url))) {
    ...
    if (content.includes('@tauri-apps/')) throw new Error(`Tauri import leaked outside runtime adapters: ${path.pathname}`);
  }
}
...
if (tauriConfig.version !== rootPackage.version || tauriConfig.bundle?.active !== true) {
  throw new Error('Desktop bundle version or configuration is incomplete.');
}
