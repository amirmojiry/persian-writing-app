...
function requestCode(string $email = 'adult@example.com', string $device = 'device-1'): void
{
    test()->postJson('/api/v1/auth/otp/request', ['email' => $email, 'deviceId' => $device])
        ->assertAccepted()
        ->assertExactJson(['message' => 'If the address can receive mail, a code has been sent.']);
}
...
it('stores a hashed short-lived OTP and issues a scoped Sanctum token', function (): void {
    ...
    expect($challenge->code_hash)->not->toBe('1234')
        ->and(Hash::check('1234', $challenge->code_hash))->toBeTrue()
        ->and($challenge->expires_at->diffInMinutes(now()))->toBeLessThanOrEqual(5);

    $token = $this->postJson('/api/v1/auth/otp/verify', [
        'email' => 'adult@example.com', 'code' => '1234', 'deviceId' => 'device-1',
    ])->assertOk()->json('token');

    $this->withToken((string) $token)->getJson('/api/v1/me')
        ->assertOk()->assertJsonPath('user.email', 'adult@example.com');
});
...
it('rejects expired, replayed and attempt-limited codes with a generic response', function (): void {
    ...
    $this->postJson('/api/v1/auth/otp/verify', [
        'email' => 'adult@example.com', 'code' => '1234', 'deviceId' => 'device-1',
    ])->assertUnprocessable()->assertJsonPath('message', 'The code is invalid or expired.');

    config()->set('auth.otp_test_code', '4321');
    requestCode();
    for ($attempt = 0; $attempt < 5; $attempt++) {
        $this->postJson('/api/v1/auth/otp/verify', [
            'email' => 'adult@example.com', 'code' => '0000', 'deviceId' => 'device-1',
        ])->assertUnprocessable();
    }
    $this->postJson('/api/v1/auth/otp/verify', [
        'email' => 'adult@example.com', 'code' => '4321', 'deviceId' => 'device-1',
    ])->assertUnprocessable();

    config()->set('auth.otp_test_code', '1234');
    requestCode('second@example.com', 'device-2');
...
it('invalidates an older active code and keeps request responses enumeration resistant', function (): void {
    ...
    $this->postJson('/api/v1/auth/otp/verify', [
        'email' => 'known@example.com', 'code' => '1111', 'deviceId' => 'd1',
    ])->assertUnprocessable();
    $this->postJson('/api/v1/auth/otp/verify', [
        'email' => 'known@example.com', 'code' => '2222', 'deviceId' => 'd1',
    ])->assertOk();
});
...
it('accepts consented batches idempotently and blocks non-consented items', function (): void {
    ...
    $accepted = [
        ...
        'consentSnapshot' => ['accountSync' => true],
    ];
    $blocked = [
        ...$accepted,
        'idempotencyKey' => '00000000-0000-4000-8000-000000000003',
        'consentSnapshot' => ['accountSync' => false],
    ];

    $this->withToken($token)->postJson('/api/v1/sync/batch', ['items' => [$accepted, $blocked]])
        ->assertOk()
        ->assertJsonPath('items.0.status', 'accepted')
        ->assertJsonPath('items.1.status', 'blocked_privacy');

    $this->withToken($token)->postJson('/api/v1/sync/batch', ['items' => [$accepted]])
        ->assertOk()->assertJsonPath('items.0.status', 'duplicate');
