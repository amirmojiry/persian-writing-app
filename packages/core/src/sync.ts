...
export function createOutboxEntry(input: {
  ...
  return {
    ...input,
    state: input.consentSnapshot.accountSync ? 'pending' : 'blocked_privacy',
    attempts: 0,
    createdAt: input.now,
    updatedAt: input.now
  };
...
export class SyncCoordinator {
  ...
  async flush(input: { readonly online: boolean; readonly consent: ConsentSnapshot; readonly now: string }): Promise<number> {
    if (!input.consent.accountSync) {
      await this.repository.blockAllForPrivacy(input.now);
      return 0;
    }
    if (!input.online) {
      return 0;
    ...
    const sending = entries.map((entry) => ({
      ...entry,
    ...
    try {
      ...
      await this.repository.mark(sending.map((entry) => {
        ...
        return {
          ...entry,
          state: status === 'accepted' || status === 'duplicate' ? 'sent' : status,
          updatedAt: input.now,
          lastError: status === 'failed' ? 'Sync endpoint rejected the item.' : undefined
        };
      }));
...
export class InMemorySyncOutboxRepository implements SyncOutboxRepository {
  ...
  async blockAllForPrivacy(now: string): Promise<void> {
    for (const entry of this.entries.values()) {
      if (entry.state !== 'sent') {
        this.entries.set(entry.id, { ...entry, state: 'blocked_privacy', updatedAt: now });
      }
    }
  }

  snapshot(): readonly SyncOutboxEntry[] {
